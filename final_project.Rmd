...
Results from the analysis show that while in the `mtcars` dataset fuel efficiency has an apparently significant association with transmission type, it is only marginal and not directly linked. When controlling for other factors such as vehicle weight, number of cylinders, and displacement the association between fuel efficiency and transmission type all but disappears. These confounding factors are closely correlated with one another, which logically makes sense. 

## Motivating Questions

The editors at _Motor Trend_ have posed two motivating questions:

  __Is an automatic or manual transmission better for MPG?__

  __Quantify the MPG difference between automatic and manual transmissions.__

In order to better prepare to answer these questions, we'll start with a survey of the data and the features describing each observation.
...
Two of the features most closely correlated with transmission, number of cylinders and displacement, would intuitively be close covariates since the latter is naturally a function of the former. A simple scatter plot ([_fig. 3_](#figure-3)) shows a clear pattern and confirms this intuition. While there are some automatic cars with small 4 cylinders and small displacements there are far more in with large diplacements and 8 cylinders, both categories correlated with low fuel efficiency.

A similar pattern can be seen with weight. A plot of displacement by weight with points colored by number of cylinders shows how closely the three variables are correlated ([_fig. 4_](#figure-4)). All three variable track neatly together. For this reason, only the one that is most closely associated with fuel efficiency will be used. The correlations for weight, displacement, and number of cylinders respectively are `r round(cor(mtcars$wt, mtcars$mpg), 3)`, `r round(cor(mtcars$disp, mtcars$mpg), 3)`, and `r round(cor(mtcars$cyl, mtcars$mpg), 3)`.

The intuition is that although it appeared to be strongly correlated to fuel efficiency in the naive model, transmission type will be insignificant when weight is added to the model. 
...
Using an additive error, multivariable linear model (_$Y_i$ = $\beta_{tr}X_{i_{tr}}$ + $\beta_{wt}X_{i_{wt}}$_) with weight added as a predictor the coefficient for automatic transmission is near zero ($\beta_{atr}$ = `r round(coef(fit2)[[2]], 3)`) indicating a neglible difference in the average fuel efficiency between automatic and manual transmission vehicles featured in the dataset when weight is accounted for. Based on the strong correlation that we established between weight and number of cylinders and displacement, we could expect similar results if we included either of the latter two in place of weight. For the same reasons, and to keep the variance of the standard error as low as possible, we'll leave the latter two out of the model.
...
To further test acceleration as a model component for explaining some of the variation not covered by weight and transmission type, and seen in the outliers, we'll contruct a third model that includes acceleration. 
...
The findings suggest that, for reasons that aren't clear in the data, transmission type is also closely related to the three confounders. This highlights the primary limitation of the dataset and the analysis, a dataset with relatively few observations.

In response to the motivating questions posed by _Motor Trend_, within the context of the `mtcars` dataset and without controlling for other variables, automatic transmission is associated with worse gas mileage not better. The difference was captured in our first, naive linear model which calculated the difference in means to be `r coef(fit1)[1]`mpg. When including a weight in the linear model there is almost no apparent relationship between fuel efficiency and transmission type. The final model was further improved by including acceleration as a second principle component, and by removing a influential outlier.
